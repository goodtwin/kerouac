...
function Kerouac() {
  ...
  this._stack = [];
  this._cache = {};
  this._press = new Press();
  this.__defineGetter__('press', function() {
    this._usedPress = true;
    return this._press.middleware;
  });
 ...
 *
 * By default will Kerouac `require()` the engine based on the file extension.
 ...
 *
 * By default, Kerouac will parse front matter in JSON or YAML format.  It is
...
Kerouac.prototype.page = function(path, fns) {
  ...
  if (!this._usedPress) this.use(this.press);
  var type = this._press.type.apply(this._press, args);
  if (type.isSet()) {
...
Kerouac.prototype.content = function(path) {
  ...
  diveSync(adir, function(err, file) {
    ...
    if( comps[ comps.length -1 ].match( /md|markdown/ig ) ){
